...
# Автоматически пересоздаём схему перед каждым тестом
...
@pytest_asyncio.fixture
async def payment(session, user, subscription):
...
@pytest_asyncio.fixture
async def workflow(session, user):
    return await user_workflow_crud.create_user_workflow(
        session,
        user_id=user.id,
        workflow_id="test",
        name="wf"
...
async def account(session, user):
    return await social_account_crud.create_social_account(
        session,
        user_id=user.id,
        platform="telegram",
        channel_name="ch",
...
@pytest_asyncio.fixture
async def post(session, workflow):
...
@pytest_asyncio.fixture
async def stats(session, post):
    return await post_stats_crud.create_post_stats(
        session,
        post_id=post.id,
        views=100,
        likes=5
...
async def test_user_crud(session: AsyncSession):
    user = await crud.user_crud.create_user(session, telegram_id=123456, name="Test User", language="en")
    assert user.id
...
async def test_subscription_crud(session: AsyncSession, user: User):
    ...
    sub = await crud.subscription_crud.create_subscription(
        session, user_id=user.id, type='basic', start_date=now, end_date=now + timedelta(days=30)
    )
    assert sub.id
...
async def test_payment_crud(session: AsyncSession, user: User, subscription: Subscription):
    pay = await crud.payment_crud.create_payment(session, user_id=user.id, subscription_id=subscription.id, amount=1000)
    assert pay.id
...
async def test_social_account_crud(session: AsyncSession, user: User):
    account = await crud.social_account_crud.create_social_account(
        session, user_id=user.id, platform="telegram",
        channel_name="Test", channel_id="1234", channel_type="public"
    )
    assert account.id
...
async def test_user_workflow_crud(session: AsyncSession, user: User):
    wf = await crud.user_workflow_crud.create_user_workflow(session, user_id=user.id, workflow_id="w123", name="Workflow")
    assert wf.id
...
async def test_post_stats_crud(session: AsyncSession, post: Post):
    stats = await crud.post_stats_crud.create_post_stats(session, post_id=post.id, views=100, likes=5)
    assert stats.id
